...
exports.getSensor = function(req, res, next) {
  getDronesFromUser(req.headers['user-email'], function(drones, err) {
    ...
    if (foundDrone) {
      ...
      if (!drone.sensors[drone._id.toString()]) {
        return res.status(400).json({error: "No sensor information for this drone."});
      }

      if (!drone.sensors[drone._id.toString()][req.params.payload]) {
        return res.status(400).json({error: "No sensor information for " + req.params.sensor});
      }
      return res.json(drone.sensors[drone._id.toString()][req.params.payload]);
    } else {
...
exports.updateSensor = function(req, res, next) {
  getDronesFromUser(req.headers['user-email'], function(drones, err) {
    ...
    if (foundDrone) {
      if (!drone.sensors[drone._id]) {
          drone.sensors[drone._id] = {};
      }

      if (!drone.sensors[drone._id][req.params.payload]) {
        drone.sensors[drone._id][req.params.payload] = req.body;
      }
  ...
  Drone
    ...
    .exec(function(err, drone) {
      ...
      if (!drone) {
        ...
        if (!drone.sensors[drone._id.toString()]) {
            drone.sensors[drone._id.toString()] = {};
        }

        if (!drone.sensors[drone._id.toString()][req.params.payload]) {
          drone.sensors[drone._id.toString()][req.params.payload] = req.body;
        }
