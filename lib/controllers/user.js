    ...
    User
      ...
      .exec(function(err, user) {
        if (err || !user) {
          return res
            .status(400)
            .json({error: err || "Please enter a valid email"});
        }
            ...
            res.json(
              {
                ...
                users: utils.getPublicUserData(users)
              });
          }
        });
      }
...
exports.find = function(req, res) {
  User.findOne({_id: utils.castDocumentId(req.params.id)}, function(err, user) {
    if (err) {
  ...
  User
    .findOne(
      ...
      function(err, data) {
        if (err || data) {
          return res.status(400)
                    .json({message: err || "Email is already taken"});
        }
        else {
          ...
          user.save(function(err) {
            if (err)
              ...
              return res.status(200)
                        .json({message: "New user is created"});
          });
        }
    })
};
