...
exports.generateUser = function(req, res, next) {
  ...
  if (!req.body.email || !req.body.password || !req.body.firstname
    || !req.body.lastname || !req.body.phone || !req.body.country
    || !req.body.company) {
    return res.status(400).json({error: "The following fields are required: email, password, firstname, lastname, company, phone, country"});
  }
  ...
  User
    ...
    .exec(function(err, user) {
      if (err || !user) {
        ...
        var decoded = Buffer.from(req.body.password, 'base64');
  ...
  User
    ...
    .exec(function(err, user) {
      if (err || !user) {
        ...
        sms.SMSAuth(user.email, user.phone, user.country, function(ret, err) {
          if (!err && ret && ret.success) {
            return res.json(ret);
          } else if (ret && !ret.success) {
            return res.status(400).json({error: ret});
