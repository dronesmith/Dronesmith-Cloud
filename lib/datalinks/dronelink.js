    ...
    mavListener.on('message', function(msg, rinfo) {
      ...
      function handleOpCode(data, sess) {
        ...
        if (sessionsHash[sess]) {
          ...
          global.KeenTracking.recordEvent('dronelink', {
            env: global.KEEN_ENV,
            event: 'code_update',
            session: statusObj
          });
      ...
      function handleOpStatus(data, sess, addr, port) {
        ...
        if (sessionsHash[sess]) {
          ...
          if (data.sensors && statusObj.drone && MavData[statusObj.drone._id]) {
            if (!MavData[statusObj.drone._id]['PAYLOAD']) {
              MavData[statusObj.drone._id]['PAYLOAD'] = {};
            }

            MavData[statusObj.drone._id]['PAYLOAD'] = data.sensors;
          }
      ...
      function handleBinaryMavlink(data, sess) {
        ...
        if (sessionsHash[sess]) {
          ...
          if (res) {
                ...
                if (MavData[statusObj.drone._id] == null) {
                  MavData[statusObj.drone._id] = {};
                }

                MavData[statusObj.drone._id][res.header] = res.data;
      ...
      emitter.live = function(drone) {
        for (var key in sessionsHash) {
          var statusObj = sessionsHash[key];

          if (statusObj && statusObj.drone) {
            if (statusObj.drone._id == ''+drone._id) {
              return MavData[statusObj.drone._id];
            }
          }
        }
