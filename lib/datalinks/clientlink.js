  ...
  io.on('connection', function(socket) {
    ...
    var hb = setInterval(function() {
      ...
      if (session.onlineDrones === null) {
        log.warn("[IO] Session table null?");
        session.onlineDrones = initSessionTable(session);
      }
    ...
    function onSessionUpdate(data) {
      ...
      // console.log("[IO] Got session update");
    ...
    function onMavlinkData(data) {
      if (session.onlineDrones === null) {
        ...
        session.onlineDrones = initSessionTable(session);
      } else if (session.onlineDrones[data.drone]
        && session.onlineDrones[data.drone].session == data.session) {
